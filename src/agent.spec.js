...
/* ethers mocking */
// ICHI Vault Factory contract mock and pool mock
...
const mockFactoryContract = {
  getVault: jest.fn().mockResolvedValue(mockPoolAddress),
};
...
const { abi: vaultAbi } = require("../abi/ICHIVault.json");

mockVaultContract.interface = new ethers.utils.Interface(vaultAbi);

const vaultCreatedTopic =
...
describe("large flash swap monitoring", () => {
  describe("handleTransaction", () => {
    ...
    const logsNoMatchEvent = [{ topics: [vaultCreatedTopic] }];
    ...
    it("returns empty findings if no flash swaps occurred", async () => {
      ...
      expect(mockVaultContract.token0).toHaveBeenCalledTimes(0);
    });
    ...
    it("returns empty findings a Flash event occurred for a non-Uniswap V3 pool ", async () => {
      ...
      expect(findings).toStrictEqual([]);
      expect(mockVaultContract.token0).toHaveBeenCalledTimes(1);
      expect(mockVaultContract.token1).toHaveBeenCalledTimes(1);
      expect(mockVaultContract.fee).toHaveBeenCalledTimes(1);
      mockVaultContract.token0.mockClear();
      mockVaultContract.token1.mockClear();
      mockVaultContract.fee.mockClear();
    });
